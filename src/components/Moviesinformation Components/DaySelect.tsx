    ...
    const fetchCinemas = async () => {
      try {
        const response =await axios.get(
          `${buildApiUrl(
            API_CONFIG.BACKEND.CINEMA.GET_DANH_SACH_CHIEU
          )}?movieid=${encodeURIComponent(title as string)}`
        );
    ...
    <div className="w-full flex flex-col items-center justify-center  to-black mt-40 px-4">
      ...
      <div className="flex items-center cursor-pointer justify-between w-full max-w-5xl mb-4 gap-4">
        <i
          ...
          className="fa-solid text-white fa-2xl fa-circle-arrow-left"
        ></i>
        <div
          ref={slider}
          className="flex flex-row  gap-4 overflow-x-hidden max-w-5xl w-full pb-2"
        >
          {Day.map((item, index) => (
            <div
              key={index}
            ...
            >
              <div className={`font-bold mb-2 text-[--color-brand-pink]`}>
                {item.date}
              </div>
        ...
        <i
          ...
          className="fa-solid fa-2xl cursor-pointer text-white fa-circle-arrow-left fa-flip-horizontal"
        ></i>
      </div>

      {Day.length > 0 && (
        <button
          className="mt-6 transition-colors px-8 py-3 rounded-xl text-white font-semibold bg-gradient-to-r  hover:opacity-90"
          style={{
            backgroundImage:
