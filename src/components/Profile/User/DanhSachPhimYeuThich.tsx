...
export const DanhSachPhimYeuThich = () => {
  ...
  const handleClickDelete=async (movieId:string) => {
    ...
    const token=await getToken();
    try {
      await axios.delete(`https://backendformoviebooking-production.up.railway.app/api/Client/DeleteUserFavorite?movieTitle=${encodeURIComponent(movieId)}`, {
        headers: {
          Authorization: `Bearer ${token}`
        },
      });
    ...
    <div className="p-4">
      <div className="flex flex-row items-center w-full  justify-between ">
           ...
           {Edit?<button onClick={()=>setEdit(false)} className="bg-red-500 mb-6 text-white px-4 py-2 rounded">Hủy</button>:
           <i onClick={() => setEdit(!Edit)} className="fa-solid fa-xl fa-pen-to-square mb-6 text-white"></i>}
              ...
              <div className="relative overflow-hidden rounded-lg shadow-lg">
                ...
                <div className="absolute inset-0  bg-opacity-0 group-hover:bg-opacity-30 transition-all duration-300 flex items-end">
                  <div className="p-3 w-full transform translate-y-full flex flex-row justify-between  group-hover:translate-y-0 transition-transform duration-300">
                    ...
                    <h3>
                      {Edit?<i onClick={() => handleClickDelete(movie.title)} className="fa-solid fa-xs  text-red-500 fa-trash-can p-2 rounded-2xl bg-gray-500"></i>:null}
                    </h3>
                  </div>
                </div>
              </div>
